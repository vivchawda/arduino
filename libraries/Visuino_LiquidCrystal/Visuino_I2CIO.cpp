...
int I2CIO::begin (  uint8_t i2cAddr )
{
   ...
   _initialised = FWire.requestFrom ( _i2cAddr, (uint8_t)1 );

#if (ARDUINO <  100)
   _shadow = FWire.receive ();
#else
   _shadow = FWire.read (); // Remove the byte read don't need it.
#endif
...
uint8_t I2CIO::read ( void )
{
   ...
   if ( _initialised )
   {
      FWire.requestFrom ( _i2cAddr, (uint8_t)1 );
#if (ARDUINO <  100)
      retVal = ( _dirMask & FWire.receive ( ) );
#else
      retVal = ( _dirMask & FWire.read ( ) );
#endif      
...
int I2CIO::write ( uint8_t value )
{
   ...
   if ( _initialised )
   {
      ...
      FWire.beginTransmission ( _i2cAddr );
#if (ARDUINO <  100)
      FWire.send ( _shadow );
#else
      FWire.write ( _shadow );
#endif  
      status = FWire.endTransmission ();
   }
   return ( (status == 0) );
}
